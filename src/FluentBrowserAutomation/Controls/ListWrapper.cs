    ...
    public class ListWrapper : BasicInfoWrapper
    {
        ...
        public IEnumerable<ListItemWrapper> Rows()
        {
            var items = Element.GetChildElementsByTagName("li")
                .Select((x, i) =>
					new ListItemWrapper(new RemoteWebElementWrapper(() => Element.GetChildElementsByTagName("li").ToArray()[i],x), String.Format("{0}, item with index {1}", HowFound, i), BrowserContext));
